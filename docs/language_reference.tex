...
(thus it requires you to keep them balenced). Comments do not get past
lexical analysis.
...
\end{lstlisting}
These are \enquote{reserved names} in Vortex. They cannot be used as
...
\begin{lstlisting}[language=bnf]
hexnum ::= ('0x' | '0X') { hexdigit } [ '.' ] { hex_digit }
    [ ('p' |'P') [ '+' |'-' ] digit ]
