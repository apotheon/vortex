...
function call and so on). The rationale for this is that doing it otherwise
doesn't really make sense -- allowing inclusion of arbitrary expressions
in blocks would result in code doing nothing, and that's better filtered
out at compile time.
...
\begin{lstlisting}[language=bnf]
binexp ::= exp binop exp | (ident | indexp) assop exp
unexp  ::= unop exp
exp    ::= binexp | unexp
\end{lstlisting}
...
exp ::= primaryexp
\end{lstlisting}
Primary expressions are simple expressions that can have a postfix. A postfix
is for example a parameter list (a call), brackets with an expression (index)
or a dot (simple index). Primary expressions typically don't have a side
effect, thus they can't be used in statement form unless postfixed as a call.
You can wrap any arbitrary expression in parens to get a primary expression.
All types of simple literals as well as table and list literals and macro
expansions are primary expressions.
\subsection{Suffixed expressions}
\begin{lstlisting}[language=bnf]
callpostfix ::= '(' exprlist ')'
postfix     ::= [ postfix ] ('.' ident | ':' ident callpostfix
    | callpostfix | tableexp | strliteral | ('[' expr ']'))
suffixedexp ::= primaryexp postfix
exp         ::= suffixedexp
statexp     ::= suffixedexp (':' ident callpostfix | callpostfix)
\end{lstlisting}
Suffixed expressions are primary expressions with a suffix. A suffix can
represent indexing, method calls, regular calls.
